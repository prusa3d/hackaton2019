...
//=============VARIABLES=================
	...
	uint16_t FW_build = 0;
	uint8_t suff_idx = 5;
	const char* FW_suffix_array[] = {"prealpha", "alpha", "beta", "RC", "final", NULL};
...
//=============FW VERSION PARSING=============
	...
	//stage number - 0:prealpha, 1:alpha, 2:beta, 3:release cand., 4:final
	if(FW_STAGENR < 5)
		suff_idx = FW_STAGENR;
	...
	if (suff_idx < 5)
	{
	 	sprintf(FW_version_str, "%d.%d.%d-%s+%d",
				...
				FW_suffix_array[suff_idx], FW_build);
	} else {
