...
int screen_PID_event(screen_t* screen, window_t* window, uint8_t event, void* param)
{
	...
	if (event == WINDOW_EVENT_CLICK)
		switch ((int)param){
		case TAG_QUIT:
			if (pd->autotune_state != AT_idle) return 0;//button should not be accessible
			screen_close();
			return 1;
	...
	if (event == WINDOW_EVENT_LOOP)
	{
		...
		if (pd->redraw){
			...
			display->fill_rect(rect_ui16(pd->dot_coordsKp_E[0],
					pd->dot_coordsKp_E[1],2,2), COLOR_WHITE);
			display->fill_rect(rect_ui16(pd->dot_coordsKi_E[0],
					pd->dot_coordsKi_E[1],2,2), COLOR_WHITE);
			display->fill_rect(rect_ui16(pd->dot_coordsKd_E[0],
					pd->dot_coordsKd_E[1],2,2), COLOR_WHITE);
			...
			display->fill_rect(rect_ui16(pd->dot_coordsKp_B[0],
					pd->dot_coordsKp_B[1],2,2), COLOR_WHITE);
			display->fill_rect(rect_ui16(pd->dot_coordsKi_B[0],
					pd->dot_coordsKi_B[1],2,2), COLOR_WHITE);
			display->fill_rect(rect_ui16(pd->dot_coordsKd_B[0],
					pd->dot_coordsKd_B[1],2,2), COLOR_WHITE);
...
void enable_digits_write_mode(int16_t *ids, size_t sz){
	for (size_t i = 0; i < sz; ++i)	window_enable(ids[i]);
}

void disable_digits_write_mode(int16_t *ids, size_t sz){
	for (size_t i = 0; i < sz; ++i)	window_disable(ids[i]);
}
...
void window_list_RW_item(window_list_t* pwindow_list, uint16_t index,
		const char** pptext, uint16_t* pid_icon)
{
	if (index < list_RW_strings_sz)	*pptext = list_RW_strings[index];
	else *pptext = "Index ERROR";
	*pid_icon = 0;
}
