...
constexpr size_t count_digits(int32_t i)
{
	return is_negative(i) ?
			count_digits_unsigned(-i) + 1 : count_digits_unsigned(i);
}

//n must be valid
static constexpr char valid_nth_char_unsigned(uint32_t ui, size_t n)
{
	return n == count_digits_unsigned(ui)-1 ?
			(ui % 10)+'0' : valid_nth_char_unsigned(ui/10, n);
}

//return '/0' if n is too big
static constexpr char nth_char_unsigned(uint32_t ui, size_t n)
{
	return n >= count_digits_unsigned(ui) ?
			0 : valid_nth_char_unsigned(ui, n);
}
