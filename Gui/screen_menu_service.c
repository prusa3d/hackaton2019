...
void screen_menu_service_init(screen_t* screen)
{
    ...
    psmd->items[MI_FW_UPDATE] = (menu_item_t) { { "FWupdt", 0, WI_SWITCH, .wi_switch_select = { 0, opt_enable_disable } }, SCREEN_MENU_NO_SCREEN };
    psmd->items[MI_FW_UPDATE_ON_RESTART] = (menu_item_t) { { "FWuprst", 0, WI_SWITCH, .wi_switch_select = { 0, opt_enable_disable } }, SCREEN_MENU_NO_SCREEN };
#ifdef PIDCALIBRATION
    ...
    psmd->items[MI_FW_UPDATE].item.wi_switch_select.index = sys_fw_update_is_enabled() ? 1 : 0;
    psmd->items[MI_FW_UPDATE_ON_RESTART].item.wi_switch_select.index = sys_fw_update_on_restart_is_enabled() ? 1 : 0;
}
...
int screen_menu_service_event(screen_t* screen, window_t* window, uint8_t event, void* param)
{
    ...
    if (event == WINDOW_EVENT_CLICK) {
        ...
        case MI_SSCG:
            ...
            break;
        case MI_FW_UPDATE:
            if (psmd->items[MI_FW_UPDATE].item.wi_switch_select.index) {
                sys_fw_update_enable();
            } else {
                sys_fw_update_disable();
            }
            break;
        case MI_FW_UPDATE_ON_RESTART:
            if (psmd->items[MI_FW_UPDATE_ON_RESTART].item.wi_switch_select.index) {
            ...
            break;
        case MI_BSOD:
            bsod("Congratulations. You just broke the printer.");
            break;
