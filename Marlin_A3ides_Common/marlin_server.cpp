...
int marlin_server_idle(void)
{
	if (marlin_server.idle_cnt < MARLIN_IDLE_CNT_BUSY)
		...
		if ((marlin_server.flags & MARLIN_SFLG_BUSY) == 0)
		{
			//_dbg("SVR: BUSY");
			marlin_server.flags |= MARLIN_SFLG_BUSY;
...
int _send_notify_to_client(osMessageQId queue, variant8_t msg)
{
	...
	if (queue == 0) return 0;
	if (osMessageAvailableSpace(queue) < 2) return 0;
...
void _send_notify_event(uint8_t evt_id, uint32_t usr32, uint16_t usr16)
{
	if ((marlin_server.notify_events & ((uint64_t)1 << evt_id)) == 0) return;
...
void _server_update_pqueue(void)
{
	if ((marlin_server.pqueue_head != planner.block_buffer_head) ||
			(marlin_server.pqueue_tail != planner.block_buffer_tail))
	{
		marlin_server.pqueue_head = planner.block_buffer_head;
		marlin_server.pqueue_tail = planner.block_buffer_tail;
		marlin_server.pqueue = (marlin_server.pqueue_head >= marlin_server.pqueue_tail)?
				(marlin_server.pqueue_head - marlin_server.pqueue_tail):
				(BLOCK_BUFFER_SIZE + marlin_server.pqueue_head - marlin_server.pqueue_tail);
...
uint64_t _server_update_vars(uint64_t update)
{
	...
	{
		v.ui8 = 0;
		if (stepper.axis_is_moving(X_AXIS)) v.ui8 |= 0x01;
		if (stepper.axis_is_moving(Y_AXIS)) v.ui8 |= 0x02;
		if (stepper.axis_is_moving(Z_AXIS)) v.ui8 |= 0x04;
		if (stepper.axis_is_moving(E_AXIS)) v.ui8 |= 0x08;
...
int _process_server_request(char* request)
{
	...
	if ((client_id < 0) || (client_id >= MARLIN_MAX_CLIENTS)) return 1;
...
void onStatusChanged(const char * const msg)
{
	...
	else if (strcmp(msg, "Probing failed") == 0)
...
void host_action_prompt_show()
{
	...
	marlin_host_prompt_t prompt = {
		...
		host_prompt_buttons,
		{
			marlin_host_prompt_button_by_text(host_prompt_button[0]),
			marlin_host_prompt_button_by_text(host_prompt_button[1]),
			marlin_host_prompt_button_by_text(host_prompt_button[2]),
			marlin_host_prompt_button_by_text(host_prompt_button[3]),
		}
	};
	int paused = 0;
