...
void CardReader::getAbsFilename(char *dst) {
    *dst++ = '/';
    ...
    if (cnt < MAXPATHNAMELENGTH -
                  (FILENAME_LENGTH)-1) { // Leave room for filename and nul
        appendAtom(file);
        --dst;
    }
    *dst = '\0';
...
bool CardReader::is_dir_or_gcode(const dir_t &p) {
    ...
    if (pn0 == DIR_NAME_FREE ||
        pn0 == DIR_NAME_DELETED                 // Clear or Deleted entry
...
class Slot {
  public:
    ...
    Slot() : type(File), file(), is_used(false) {
    }
...
bool SdBaseFile::seekSet(const uint32_t pos) {
    if ((pos == 0) &&
        ((type_ == FAT_FILE_TYPE_ROOT_FIXED) ||
         (type_ == FAT_FILE_TYPE_ROOT32) || (type_ == FAT_FILE_TYPE_SUBDIR))) {
