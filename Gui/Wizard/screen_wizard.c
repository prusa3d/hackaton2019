    ...
    if (event == WINDOW_EVENT_LOOP) {
        if (inside_handler == 0) {
            ...
            case _STATE_START:
                if (wizard_msgbox(
                        "Hi, this is your "
                        "Original Prusa Buddy "
                        "printer. I would like "
            ...
            case _STATE_SELFTEST_PASS:
                ...
                if (is_state_in_wizard_mask(_STATE_XYZCALIB_INIT)) //run XYZ
                    ...
                    wizard_msgbox(
                        "Everything is alright. ", MSGBOX_BTN_DONE, IDR_PNG_icon_pepa);
                pd->state = _STATE_XYZCALIB_INIT;
                break;
            case _STATE_SELFTEST_FAIL:
                wizard_msgbox(
                    "Neco se bohuzel nepovedlo behem selftestu. "
                    "Radsi se na to ...",
                    MSGBOX_BTN_DONE, 0);
                screen_close();
                break;
            ...
            case _STATE_XYZCALIB_FAIL:
                wizard_msgbox(
                    "Neco se bohuzel nepovedlo behem xyz kalibrace. "
                    "Radsi se na to ...",
                    MSGBOX_BTN_DONE, 0);
                screen_close();
                break;
            ...
            case _STATE_FIRSTLAY_MSBX_CALIB: {
                wizard_msgbox(
                    "Now I will calibrate "
                    "distance between tip of "
                    "the nozzle and heatbed "
                    "surface.",
                    MSGBOX_BTN_NEXT, 0);
                ...
                if ((diff <= -z_offset_step) || (diff >= z_offset_step)) {
                    ...
                    snprintf(buff, sizeof(buff) / sizeof(char),
                        "Do you want to use actual value? "
                        "Actual:  %0.3f.   "
                        "Default: %0.3f.   "
            ...
            case _STATE_FIRSTLAY_MSBX_START_PRINT:
                wizard_msgbox(
                    "I will start to print "
                    "line and you will "
                    "gradually lower the "
                    "nozzle by rotating the "
                    "knob until you reach "
                    "optimal height. "
                    "Check the pictures in "
                    "our handbook in chapter "
                    "Calibration.",
                    MSGBOX_BTN_NEXT, 0);
            ...
            case _STATE_FIRSTLAY_PRINT:
                if (wizard_firstlay_print(frame_id, p_firstlay_screen, p_firstlay_data, vars->z_offset) == 100)
                    pd->state = _STATE_FIRSTLAY_MSBX_REPEAT_PRINT;
                break;
            case _STATE_FIRSTLAY_MSBX_REPEAT_PRINT:
                if (wizard_msgbox(
                        "Do you want to repeat "
                        "last step to readjust "
            ...
            case _STATE_FIRSTLAY_FAIL:
                wizard_msgbox(
                    "Neco se bohuzel nepovedlo behem first layer kalibrace. "
                    "Radsi se na to ...",
                    MSGBOX_BTN_DONE, 0);
                screen_close();
                break;
